...
store_path=$1
image_info="$store_path/nix-support/image-info.json"
...
process_image () {
    ...
    if [[ -n "$image_mount" ]]; then
      image_logical_bytes=$(read_image_info ".disks.${image_mount}.logical_bytes")
      image_file=$(read_image_info ".disks.${image_mount}.file")
    else
    ...
    [[ -n "$image_mount" ]] && image_name+="-${image_mount}"
    image_description="NixOS ${image_label} ${image_system}"
    [[ -n "$image_mount" ]] && image_description+=" $image_mount"
    ...
    [[ -n "$image_mount" ]] && log " Mount Point: $image_mount"
...
if jq -e .disks "${image_info}" &> /dev/null; then
  for mount_point in $(jq -r '.disks | keys[]' "${image_info}"); do
    process_image "${mount_point}"
  done
else
  process_image
fi
